...
export default class ColumnManager {
    ...
    leafColumns() {
        return this._cache("leafColumns", () => {
            return this._leafColumns(this.groupedWithoutFixedColumns());
        });
    }
    ...
    groupedWithoutFixedColumns() {
        return this._cache("groupedWithoutFixedColumns", () =>
            this._groupedWithoutFixedColumns()
        );
    }

    _groupedWithoutFixedColumns() {
        const leftColumns = this.leftColumns();
        const rightColumns = this.rightColumns();
        let groupedColumns = this.groupedColumns();

        const contains = function (columns, key) {
        ...
        if (leftColumns && leftColumns.length > 0) {
            groupedColumns = groupedColumns.filter(
                (column) => !contains(leftColumns, column.key)
            );
        }

        if (rightColumns && rightColumns.length > 0) {
            groupedColumns = groupedColumns.filter(
                (column) => !contains(rightColumns, column.key)
            );
        }
    ...
    normalize(elements) {
        ...
        elements.forEach((element) => {
            if (!this.isColumnElement(element)) {
                return;
            }
